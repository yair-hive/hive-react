...
export function useSeatsGroupsData(){
    ...
    return useQuery(['seats_groups', {project_name, map_name}], new_api.seats_groups.get_all)
}
export function useSeatsGroupsCreate(){
    ...
    const mutation = useMutation(async ({data}) => {
        ...
        return new_api.seats_groups.create(name, from_row, from_col, to_row, to_col, map_name, project_name)
    }, {
        onSuccess: ()=>{
            var msg = JSON.stringify({action: 'invalidate', quert_key: ['seats_groups', {project_name}]})
            hiveSocket.send(msg)
        }
    })
