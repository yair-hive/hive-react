...
export function useMapElementsData(){
    ...
    return useQuery(['elements', {map_name, project_name}], new_api.map_elements.get_all)
}
export function useMapElementsCreate(){
    ...
    var mutation = useMutation(({data}) => {
        const {name, from_row, from_col, to_row, to_col} = data
        return new_api.map_elements.create(map_name, project_name, name, from_row, from_col, to_row, to_col)
    }, {
        onSuccess: ()=>{
...
export function useMapElementsDelete(){
    ...
    var mutation = useMutation((elements_ids) => {
